...
from noha import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER
from noha.__main__ import DATA_IMPORT
...
# from noha.modules.rules import get_rules
import noha.modules.sql.rules_sql as rulessql

# from noha.modules.sql import warns_sql as warnssql
import noha.modules.sql.blacklist_sql as blacklistsql
from noha.modules.sql import disable_sql as disabledsql

# from noha.modules.sql import cust_filters_sql as filtersql
# import noha.modules.sql.welcome_sql as welcsql
import noha.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    f = open("noha{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("noha{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `noha-Backup` was specially made for notes."
        .format(chat.title, chat_id, tgl),
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove("noha{}.backup".format(chat_id))  # Cleaning file
